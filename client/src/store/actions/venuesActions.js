...
export const getVenuesList = ({ types }) => (dispatch) => {
  axios.post('/api/venues', { types })
      .then((res) => dispatch({ type: reduxTypes.FETCH_API_SUCCESS, payload: res.data, preferences: types }))
...
export const setPreferences = ({ preferences, uid }) => (dispatch) => {
  dispatch({ type: reduxTypes.SET_PREFERENCES_SUCCESS, payload: preferences });
